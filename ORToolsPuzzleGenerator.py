...
class ORToolsPuzzleGenerator:
    ...
    def _try_generate(self, difficulty: int) -> bool:
        ...
        for clue_type, target_count in recipe.items():
            ...
            if added_count < target_count:
                print(f"  - [ВНИМАНИЕ] Не удалось добавить нужное количество ({target_count}) непротиворечивых подсказок типа '{clue_type}'. Добавлено: {added_count}.")
            else:
    ...
    def _generate_clue_pool(self) -> Dict[str, List[Tuple[str, Any]]]:
        ...
        for i_pos in range(1, self.num_items + 1):
            ...
            unique_clues['positional'].add(('positional', (i_pos, random.choice(cat_keys), row[random.choice(cat_keys)])))
...
if __name__ == '__main__':
    THEMES = {
        ...
        "Космическая Одиссея": {
            ...
            "Корабль": ["Серенити", "Нормандия", "Галактика", "Звёздный Крейсер", "Энтерпрайз", "Сокол", "Прометей", "Ностромо"],
            "Сектор": ["Орион", "Андромеда", "Плеяды", "Центавра", "Гидра", "Войд", "Квазар", "Небула"],
            "Аномалия": ["Червоточина", "Грави-колодец", "Темпоральный сдвиг", "Нейтронная буря", "Пси-поле", "Ксено-артефакт", "Сингулярность", "Эхо Пустоты"]
        },
        "Тайна в Фэнтези-Таверне": {
            "Посетитель": ["Гном-кузнец", "Эльф-лучник", "Орк-берсерк", "Человек-маг", "Хоббит-вор", "Драконид-паладин", "Полурослик-бард", "Гоблин-алхимик"],
            "Напиток": ["Драконья Горечь", "Эльфийский Эль", "Гномье Крепкое", "Орочья Брага", "Слёзы Грифона", "Сок мандрагоры", "Лунная Роса", "Гоблинский Грог"],
            "Артефакт": ["Рунный клинок", "Посох молний", "Амулет невидимости", "Кольцо регенерации", "Сапоги-скороходы", "Плащ из тени", "Свиток огня", "Философский камень"],
            "Квест": ["Убить дракона", "Найти сокровище", "Спасти принцессу", "Охранять караван", "Сварить зелье", "Расшифровать карту", "Украсть реликвию", "Победить в турнире"]
        },
        "Загадка Гранд-Отеля 'Нуар'": {
            "Гость": ["Детектив Харди", "Певица Лола", "Магнат Вандербильт", "Актриса Монро", "Гангстер Капоне", "Профессор Мориарти", "Шпионка Романова", "Контрабандист Рик"],
            "Этаж": ["Пентхаус", "Библиотека", "Бальный зал", "Джаз-клуб", "Сигарная комната", "Винный погреб", "Терраса", "Казино"],
            "Предмет": ["Жемчужное ожерелье", "Револьвер с гравировкой", "Зашифрованная записка", "Пустая ампула", "Окровавленный кинжал", "Компромат в папке", "Ключ от сейфа", "Бриллиантовое колье"],
            "Алиби": ["Играл в покер", "Слушал джаз", "Был на встрече", "Пил виски в баре", "Читал в библиотеке", "Танцевал в зале", "Был на террасе", "Делал ставку в казино"]
        }
