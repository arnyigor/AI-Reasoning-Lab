# CoreGenerator.py (рефакторинг с Enum)
...
class CorePuzzleGenerator:
    ...
    def _build_walls(self, core_puzzle: List, wall_clues: List) -> Optional[List]:
        ...
        # --- ИЗМЕНЕНИЕ: Используем Enum для разделения подсказок ---
        simple_clue_types = {ClueType.POSITIONAL, ClueType.DIRECT_LINK}

        simple_walls = [c for c in wall_clues if c[0] in simple_clue_types]
        complex_walls = [c for c in wall_clues if c[0] not in simple_clue_types]
        # --- КОНЕЦ ИЗМЕНЕНИЯ ---
    ...
    def _minimize_puzzle(self, puzzle: List[Tuple[Any, Any]], anchors: set) -> List[Tuple[Any, Any]]:
        ...
        # Преобразуем якоря в кортежи для корректного сравнения
    ...
    def _create_or_tools_model(self, clues: List[Tuple[Any, Any]]):
