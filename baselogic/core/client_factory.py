...
from baselogic.core.ollama_client import OllamaClient
from baselogic.core.openai_client import OpenAICompatibleClient, log
...
class LLMClientFactory:
    ...
    def create_provider(model_config: Dict[str, Any]) -> ProviderClient:
        ...
        log.info("  🔧 Создаем провайдера '%s' для модели '%s'...", client_type, name)
        ...
        if client_type == "ollama":
            ...
            log.info("   - Класс: OllamaClient, URL: %s", api_base)
            ...
            log.info("   - Класс: OpenAICompatibleClient, URL: %s", api_base)
            ...
            # Gemini: обязателен api_key.
            if not api_key:
                raise ValueError("Для 'gemini' клиента должен быть указан 'api_key'.")
            log.info("   - Класс: GeminiClient")
